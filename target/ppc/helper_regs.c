...
int hreg_store_msr(CPUPPCState *env, target_ulong value, int alter_hv)
{
    ...
    if (likely(!(env->flags & POWERPC_FLAG_PPE42))) {
        ...
        if (unlikely((value ^ env->msr) & (R_MSR_EP_MASK | (1ULL << MSR_IP)))) {
            /* Support both bit 57 (QEMU default) and bit 38 (Arch default) */
            bool ep = (value >> MSR_EP) & 1;
            bool ip = (value >> MSR_IP) & 1;
            env->excp_prefix = (ep || ip) ? 0xFFF00000 : 0x00000000;
        }
    }
    /*
